...
from ui.uiFileTypeDialog import Ui_FileTypeDialog
...
class WorkerThread(QThread):
    ...
    def start_schedule(self):
        for item in self.selected_items:
            process = subprocess.Popen(['python', 'backup.py', '-p', './data/'+item.text()+'.json'], shell=True, stdout=subprocess.PIPE)
            try:
                while True:
...
class AddFileMaskDialog(QtGui.QDialog, Ui_FileTypeDialog):
    def __init__(self):
...
class Presenter:
    ...
    def window_processor(self):
        ...
        self.w.projetcList.setSelectionMode(QtGui.QAbstractItemView.ExtendedSelection)
        self.w.show()

    # file Type  Dialog
    def file_mask_dialog_processor(self):
        self.FileMaskDialogWindow.buttonBox.accepted.connect(self.add_filemask)
        self.FileMaskDialogWindow.FileMaskList.setSelectionMode(QtGui.QAbstractItemView.ExtendedSelection)
    ...
    def open_folder_dialog():
            folder_name_path = QtGui.QFileDialog().getExistingDirectory(None, 'Output directory', QtCore.QDir.homePath(),
