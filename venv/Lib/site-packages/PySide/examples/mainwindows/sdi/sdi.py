...
class MainWindow(QtGui.QMainWindow):
    ...
    def __init__(self, file_name=None):
        ...
        self.init()
        if file_name:
            self.loadestination_file(file_name)
        else:
            self.setCurrentFile('')
    ...
    def open(self):
        file_name, filtr = QtGui.QFileDialog.getOpenFileName(self)
        if file_name:
            existing = self.findMainWindow(file_name)
            if existing:
                existing.show()
                existing.raise_()
    ...
    def loadestination_file(self, file_name):
    ...
    def setCurrentFile(self, file_name):
        self.isUntitled = not file_name
        if self.isUntitled:
            self.curFile = "document%d.txt" % MainWindow.sequenceNumber
            MainWindow.sequenceNumber += 1
        else:
            self.curFile = QtCore.QFileInfo(file_name).canonicalFilePath()
    ...
    def strippedName(self, fullFileName):
        return QtCore.QFileInfo(fullFileName).file_name()

    def findMainWindow(self, file_name):
        canonicalFilePath = QtCore.QFileInfo(file_name).canonicalFilePath()
