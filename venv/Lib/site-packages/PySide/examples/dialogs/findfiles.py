...
"""PyQt4 port of the dialogs/findestination_files example from Qt v4.x"""
...
class Window(QtGui.QDialog):
    ...
    def find(self):
        ...
        if not file_name:
            file_name = "*"
        files = self.currentDir.entryList([file_name],
                QtCore.QDir.Files | QtCore.QDir.NoSymLinks)

        if text:
            files = self.findestination_files(files, text)
        self.showFiles(files)

    def findestination_files(self, files, text):
        ...
        for i in range(len(files)):
            ...
            if inFile.open(QtCore.QIODevice.ReadOnly):
                ...
                while not stream.atEnd():
                    ...
                    if text in line:
                        foundestination_files.append(files[i])
                        break

        progressDialog.close()

        return foundestination_files
    ...
    def showFiles(self, files):
        for fn in files:
            ...
            file_nameItem = QtGui.QTableWidgetItem(fn)
            file_nameItem.setFlags(file_nameItem.flags() ^ QtCore.Qt.ItemIsEditable)
            ...
            self.filesTable.setItem(row, 0, file_nameItem)
