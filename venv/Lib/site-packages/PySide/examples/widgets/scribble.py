...
class ScribbleArea(QtGui.QWidget):
    ...
    def saveImage(self, file_name, fileFormat):
        visibleImage = self.image
        self.resizeImage(visibleImage, size())

        if visibleImage.save(file_name, fileFormat):
            self.modified = False
            return True
        else:
            return False
...
class MainWindow(QtGui.QMainWindow):
    ...
    def open(self):
        if self.maybeSave():
            ...
            if file_name:
                self.scribbleArea.openImage(file_name)
    ...
    def saveFile(self, fileFormat):
        ...
        file_name,_ = QtGui.QFileDialog.getSaveFileName(self, "Save As",
                initialPath,
                "%s Files (*.%s);;All Files (*)" % (str(fileFormat).upper(), fileFormat))
        if file_name:
            return self.scribbleArea.saveImage(file_name, fileFormat)
